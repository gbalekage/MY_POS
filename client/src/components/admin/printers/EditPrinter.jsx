...
const EditPrinter = ({ isOpen, onClose, onSuccess, selectedClient }) => {
  ...
  useEffect(() => {
    if (selectedClient) {
      setPrinterData({
        ...
        port: selectedClient.port || 9100,
        isDefault: selectedClient.isDefault ?? true,
        canEditPort: false,
      });
    }
  }, [selectedClient]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${serverUrl}/api/printers/${printerData._id}`,
        printerData,
        {
      ...
      toast.error(
        error.response?.data?.message ||
          "Erreur lors de la modification de l'imprimante."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Modifier une imprimante</DialogTitle>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  name="port"
                  disabled={!printerData.canEditPort || loading}
                  onChange={handleChange}
                  required
                  min={1}
                  step={1}
