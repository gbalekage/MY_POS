...
const DeletePrinter = ({ isOpen, onClose, onSuccess, selectedUser }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`${serverUrl}/api/printers/${selectedUser._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Imprimente supprimé avec succès !");
      onSuccess?.();
      onClose();
    } catch (error) {
      toast.error(
        error.response?.data?.message ||
          "Erreur lors de la suppression de l'imprimente."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Supprimer l'imprimente</DialogTitle>
          <DialogDescription>
